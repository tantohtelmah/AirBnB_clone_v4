$(document).ready(function() {
	...
	$('input[type="checkbox"]').on('change', function() {
		...
		const cityyName = $(this).data('name');

		if ($(this).prop('checked')) {
			// Checkbox is checked, add to selected amenities
			selectedAmenities.push(amenityId);
			selectedStates.push(stateIdId);
			selectedCities.push(cityIdId);
		} else {
			// Checkbox is unchecked, remove from selected amenities
			const indexA = selectedAmenities.indexOf(amenityId);
			const indexS = selectedStates.indexOf(stateId);
			const indexC = selectedC.indexOf(cityId);
			if (indexA !== -1) {
		...
		$('#selectedStates').text('Selected States: ' + selectedStates.join(', '));
		$('#selectedCities').text('Selected Cities: ' + selectedCities.join(', '));
	});
